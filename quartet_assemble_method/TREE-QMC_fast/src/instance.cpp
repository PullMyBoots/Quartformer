...
int Instance::parse(int argc, char **argv) {
    ...
    while (i < argc) {
        ...
        else if (opt == "--quartets-bin") {
            data_mode = "q";
            quartet_binary_input = true;
        }
...
void Instance::input_quartets(std::istream &in) {
    if (quartet_binary_input) {
        input_quartets_binary(in);
        return;
    }
    ...
    if (firstLine == "t1,t2,t3,t4,CF12_34,CF13_24,CF14_23,ngenes")
        input_quartets_phylonetworks(in);
    else
        input_quartets_basic(in, &firstLine);
}
...
void Instance::input_quartets_basic() {
    ...
    input_quartets_basic(fin, NULL);
    fin.close();
}


void Instance::input_quartets_basic(std::istream &in, const std::string *first_line) {

    std::vector<std::string> tokens;
    std::string delimiter = "___";
    size_t pos = 0, count = 0;
    std::string format_copy = quartet_format;
    while ((pos = format_copy.find(delimiter)) != std::string::npos) {
        count ++;
        tokens.push_back(format_copy.substr(0, pos));
        format_copy.erase(0, pos + delimiter.length());
    }
    if (format_copy != "") tokens.push_back(format_copy);
    if (count != 5) {
        std::cout << "\nERROR: Invalid quartet format " << quartet_format << std::endl;
        exit(1);
    }
...
void Instance::input_quartets_phylonetworks() {
    ...
    input_quartets_phylonetworks(fin);
    fin.close();
}


void Instance::input_quartets_phylonetworks(std::istream &in) {
    std::string line;
    size_t j = 0;
    while (std::getline(in, line)) {
        j ++;
...
void Instance::input_quartets_binary(std::istream &in) {
    struct QuartetBinaryRecord {
        uint64_t split_key;
        int64_t weight;
    };

    std::unordered_map<uint16_t, index_t> idx_cache;
    idx_cache.reserve(4096);

    QuartetBinaryRecord rec;
    while (in.read(reinterpret_cast<char*>(&rec), sizeof(rec))) {
        if (rec.weight == 0) continue;

        uint16_t raw_a = static_cast<uint16_t>((rec.split_key >> 48) & 0xFFFFULL);
        uint16_t raw_b = static_cast<uint16_t>((rec.split_key >> 32) & 0xFFFFULL);
        uint16_t raw_c = static_cast<uint16_t>((rec.split_key >> 16) & 0xFFFFULL);
        uint16_t raw_d = static_cast<uint16_t>(rec.split_key & 0xFFFFULL);

        auto map_idx = [&](uint16_t raw) -> index_t {
            auto it = idx_cache.find(raw);
            if (it != idx_cache.end()) return it->second;
            index_t mapped = dict->label2index("t" + std::to_string(raw));
            idx_cache.insert({raw, mapped});
            return mapped;
        };

        index_t indices[4];
        indices[0] = map_idx(raw_a);
        indices[1] = map_idx(raw_b);
        indices[2] = map_idx(raw_c);
        indices[3] = map_idx(raw_d);

        quartet_t quartet = join(indices);
        if (quartets.find(quartet) == quartets.end())
            quartets[quartet] = 0;
        quartets[quartet] += static_cast<weight_t>(rec.weight);
    }
    if (!in.eof()) {
        std::cout << "\nERROR: Corrupted binary quartet stream (partial record)" << std::endl;
        exit(1);
    }
}
