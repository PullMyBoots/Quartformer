...
def _load_optional_extension(module_name: str, glob_pattern: str):
    """Load an optional native extension if it has been built."""
    matches = sorted((Path(__file__).resolve().parent).glob(glob_pattern))
    if not matches:
        return None

    try:
        spec = importlib.util.spec_from_file_location(module_name, str(matches[0]))
...
def _aggregate_quartets_on_cpu(
    ...
    if quartet_aggregate is not None:
        return quartet_aggregate.aggregate_quartets(
            np.ascontiguousarray(quartets_flat, dtype=np.int32),
            np.ascontiguousarray(weights_flat, dtype=np.float64),
            num_shards=64,
        )
...
def _reduce_quartet_aggregates_on_cpu(
    ...
    if quartet_aggregate is not None:
        return quartet_aggregate.reduce_aggregates(
            np.ascontiguousarray(merged_keys, dtype=np.uint64),
            np.ascontiguousarray(merged_sums, dtype=np.float64),
            np.ascontiguousarray(merged_counts, dtype=np.int64),
            num_shards=1,
        )
...
def run_qf(
    ...
    # Load Input Data
    # ================================================================================
    seq_payload, species_names, seq_length = sp.load_phy_to_packed_tensor(
        str(phy_path), drop_conserved_sites=True
    )
    num_species = seq_payload.shape[0]
    ...
    def _flush_shard(state, shard_idx: int):
        pending_entries = state["pending_entries"]
        if pending_entries[shard_idx] == 0:
            return

        shard_agg_keys = state["shard_agg_keys"]
        shard_agg_sums = state["shard_agg_sums"]
        shard_agg_counts = state["shard_agg_counts"]
        pending_keys = state["pending_keys"]
        pending_sums = state["pending_sums"]
    ...
    def _merge_batch_into_state(state, uniq_keys, batch_sums, batch_counts):
        shard_ids = (uniq_keys & np.uint64(shard_mask)).astype(np.int32, copy=False)
        order = np.argsort(shard_ids, kind="stable")
        uniq_keys_local = uniq_keys[order]
        batch_sums_local = batch_sums[order]
        batch_counts_local = batch_counts[order]
        shard_ids_local = shard_ids[order]

        split_points = np.flatnonzero(np.diff(shard_ids_local)) + 1
        starts = np.concatenate(([0], split_points))
        ends = np.concatenate((split_points, [uniq_keys_local.size]))
        ...
        for start, end in zip(starts, ends):
            shard_idx = int(shard_ids_local[start])
            pending_keys[shard_idx].append(uniq_keys_local[start:end])
            pending_sums[shard_idx].append(batch_sums_local[start:end])
            pending_counts[shard_idx].append(batch_counts_local[start:end])
            pending_entries[shard_idx] += end - start
            if pending_entries[shard_idx] >= shard_flush_threshold:
                _flush_shard(state, shard_idx)

    def _finalize_aggregation_state(state):
        key_parts = []
        sum_parts = []
        count_parts = []
        for shard_idx in range(num_agg_shards):
            _flush_shard(state, shard_idx)
            if state["shard_agg_keys"][shard_idx].size == 0:
                continue
            key_parts.append(state["shard_agg_keys"][shard_idx])
            sum_parts.append(state["shard_agg_sums"][shard_idx])
            count_parts.append(state["shard_agg_counts"][shard_idx])

        if key_parts:
            return (
                np.concatenate(key_parts),
                np.concatenate(sum_parts, axis=0),
    ...
    aggregation_state = _init_aggregation_state()
    ...
    try:
        for i in range(0, len(blocks), infer_batch_size):
            ...
            if enable_deferred_aggregation:
                ...
                _merge_batch_into_state(aggregation_state, uniq_keys, batch_sums, batch_counts)
