...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Validate backend_v2 packed pipeline against current backend up to model logits.")
    ...
    ref_sp = _load_first_extension("sequence_processor_backend", BACKEND_DIR, "sequence_processor_backend")
    ref_pf = _load_first_extension("pattern_freq_cuda_backend", BACKEND_DIR, "pattern_freq_cuda_backend")
    fast_sp = _load_first_extension("sequence_processor_backend_v2", BACKEND_V2_DIR, "sequence_processor_backend_v2")
    fast_pf = _load_first_extension("pattern_freq_cuda_backend_v2", BACKEND_V2_DIR, "pattern_freq_cuda_backend_v2")
    ...
    failures = []
    for phy_path in phy_files:
        packed_ref, names_ref, seq_length_ref = ref_sp.load_phy_to_packed_tensor(str(phy_path), args.drop_conserved_sites)
        packed_fast, names_fast, seq_length = fast_sp.load_phy_to_packed_tensor(str(phy_path), args.drop_conserved_sites)
        unpacked_ref = ref_sp.unpack_packed_tensor(packed_ref, int(seq_length_ref))
        unpacked_fast = fast_sp.unpack_packed_tensor(packed_fast, int(seq_length))

        dense_ref_np = np.asarray(unpacked_ref)
        unpacked_fast_np = np.asarray(unpacked_fast)
        packed_ref_np = np.asarray(packed_ref)
        ...
        idx_ref_cuda = torch.as_tensor(quartets_flat, device=device, dtype=torch.int64)
        idx_fast_cuda = torch.as_tensor(quartets_flat, device=device, dtype=torch.int32)

        seq_ref_cuda = torch.from_numpy(packed_ref_np).to(device=device, dtype=torch.uint8)
        seq_fast_cuda = torch.from_numpy(packed_fast_np).to(device=device, dtype=torch.uint8)

        with torch.no_grad():
            ref_pattern = ref_pf.compute_pattern_frequencies_cuda_packed(
                seq_ref_cuda,
                idx_ref_cuda,
                int(seq_length_ref),
            ).contiguous()
