...
DEVICE = "cuda"
###############################################################################
# Hyperparameters (keep BLOCK_M equal to BLOCK_N)
BLOCK_M = 16
BLOCK_N = 16
################################################################################
# Layout utilities
...
def _get_layout_quartetmatrix(sample_quartets, device="cuda", block_size=BLOCK_M):
    ...
    def calculate_shared_species_mask(positionidx, species_threshold, pad=None):
        """
        Compute shared species attention mask.

        Args:
            positionidx: Tensor of shape (quartet_num, 128), dtype=torch.bool
            species_threshold: Minimum shared species count
            pad: If not None, pad mask size to multiple of pad

        Returns:
            Attention mask of shape (quartet_num, quartet_num) or padded version
        """
...
def _partition_bool_matrix(input_matrix, M=BLOCK_M, H=BLOCK_M, device='cuda'):
    """
    Partition boolean matrix into M x H blocks using PyTorch.

    Args:
        input_matrix: Input a x a boolean matrix
        M: Row block size
        H: Column block size
        device: Target device

    Returns:
        Partitioned (a/M) x (a/H) boolean matrix
    """
    input_matrix = input_matrix.to(device)

    a, b = input_matrix.shape
    assert a % M == 0 and b % H == 0, "Matrix dimensions must be divisible by M and H"
